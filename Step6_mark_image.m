%% 手書き文字から学習用データの作成
clear all;clc;close all;[status] = rmdir('image_mark','s');
%% スキャナーで読み取った100個の手書き文字が書かれた画像を読み込み
I = imread('image_mark.jpg');
figure,imshow(I);
%% 数値が書かれた領域のボックスを作成
BW = imbinarize(I); % 二値化
...
for n= 1:10
    ...
    % 保存用のディレクトリを作成
    if n == 10 
        mkdir image_mark\train\0
        mkdir image_mark\test\0
    else
        mkdir(['image_mark\train\' num2str(n)])
        mkdir(['image_mark\test\' num2str(n)])
    end
    ...
    for i=1:10
        ...
        % 画像を保存
        if n ==10
            imwrite(I4,['image_mark\' ftype '\0' '\' ftype '_' num2str(0) '_' num2str(fnum) '_mark.jpg'] );
        else
            imwrite(I4,['image_mark\' ftype '\' num2str(n) '\' ftype '_' num2str(n) '_' num2str(fnum) '_mark.jpg'] );
        end
        clear I2 I3 
    end
end
winopen('image_mark') % Windowsのエクスプローラでフォルダを開く
%% 
