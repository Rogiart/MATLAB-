clear all;clc;close all;[status] = rmdir('image_mix','s');
% 二人分のデータを
% image_mixフォルダを作りimage_mynumberとimage_othernumberのファイルの中身をコピーする

mkdir image_mix
copyfile image_mine image_mix
copyfile image_other image_mix
%% 層の読み込みと画像のセット
load mylayers
imds = imageDatastore('image_mix\train', 'IncludeSubfolders',true,'LabelSource','foldernames');
imdshandTrain = imds;
%[imdshandTrain,~] = splitEachLabel(imds,5,'randomized')
imdshandValidation = imageDatastore('image_mix\test', 'IncludeSubfolders',true,'LabelSource','foldernames');
%% 学習オプションの設定
...
for nn = 1:k
    ...
    axis([0 k 0 100]);
    title('Machine Learning','FontSize',12,'Interpreter','latex','FontName','Euclid')% タイトル
    xlabel('Iteration','FontSize',12,'Interpreter','latex','FontName','Euclida');% x座標ラベル
...
save('netresult_mix.mat','net*','accuracy','m')
